...
const requireNonEmptyString = (value, name) => {
    if (typeof value !== 'string' || !value.trim()) {
        throw new Error(`AMB.multifieldLookup requires ${name} to be a non-empty string`);
    }
...
const normalizeMasterField = masterField => {
    if (!masterField || typeof masterField !== 'object') {
        throw new Error('AMB.multifieldLookup requires masterField');
    }
...
const normalizeDependentField = dependentField => {
    if (!dependentField || typeof dependentField !== 'object') {
        throw new Error('AMB.multifieldLookup dependentFields entries must be objects');
    }
...
const assertRecordLookup = lookup => {
    if (!lookup || typeof lookup !== 'object') {
        throw new Error('AMB.multifieldLookup requires a record-based lookup');
    }

    if (typeof lookup.keyField !== 'string' || !lookup.keyField) {
        throw new Error('AMB.multifieldLookup requires a record-based lookup with keyField');
    }

    if (typeof lookup.load !== 'function' || typeof lookup.getByKey !== 'function') {
        throw new Error('AMB.multifieldLookup lookup must expose load() and getByKey()');
    }
};
 ...
 * A normal lookup answers "which value goes in this cell?"; a Multifield Lookup answers
...
export function createMultifieldLookup(options = {}) {
    ...
    const dependentColumn = (field, columnOptions = {}) => {
        ...
        if (!dependentField) {
            throw new Error(`AMB.multifieldLookup "${id}" does not define dependent field "${field}"`);
        }
...
export const multifieldLookup = createMultifieldLookup;
