...
describe('Italian municipalities demo dataset', () => {
    ...
    test('keeps the public demo reachable and free of hardcoded row rollback', () => {
        ...
        expect(demoSource).toContain('lookupBusy');
        expect(demoSource).not.toContain('lookupOpening');
        expect(demoSource).not.toContain('lookupOpen');
        expect(demoSource).not.toContain('LOOKUP_OPEN_DELAY');
        expect(demoSource).not.toContain('pendingLookupTimer');
        expect(demoSource).not.toContain('pendingLookupCell');
        expect(demoSource).not.toContain('pendingLookupRow');
        expect(demoSource).not.toContain('activeLookupRow');
        expect(demoSource).not.toContain('scheduleMunicipalityLookup');
        expect(demoSource).not.toContain('globalThis.setTimeout(');
        expect(demoSource).not.toContain('queueMicrotask(');
        ...
        expect(demoSource).toContain('data: municipalities');
        expect(demoSource).not.toContain('data: await municipalityLookup.load()');
