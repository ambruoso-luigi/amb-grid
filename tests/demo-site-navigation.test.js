...
describe('demo site navigation', () => {
    ...
    test('links the available framework cards to their internal demo pages', () => {
        ...
        expect(main).toContain("'frameworks.cta': 'Apri guida e demo'");
        expect(main).toContain("'frameworks.cta': 'Open guide and demo'");
        expect(main).toContain("'frameworks.javascript.meta': 'ESM / UMD · Integrazione diretta'");
        expect(main).toContain("'frameworks.javascript.description': 'Integra AMB Grid in pagine JavaScript classiche o server-rendered, senza framework obbligatori.'");
        expect(main).toContain("'frameworks.javascript.meta': 'ESM / UMD · Direct integration'");
        expect(main).toContain("'frameworks.javascript.description': 'Integrate AMB Grid into classic or server-rendered JavaScript pages without requiring a framework.'");
        expect(main).toContain("'frameworks.react.meta': 'TypeScript · Component lifecycle'");
        expect(main).toContain("'frameworks.react.description': 'Integra AMB Grid nel lifecycle del componente React, con mount e cleanup corretti.'");
        expect(main).toContain("'frameworks.vue.meta': 'Composition API · TypeScript'");
        expect(main).toContain("'frameworks.vue.description': 'Integra AMB Grid con la Composition API usando il lifecycle del componente Vue.'");
        expect(main).toContain("'frameworks.vue.description': 'Integrate AMB Grid with the Composition API through the Vue component lifecycle.'");
        expect(main).toContain("'frameworks.angular.meta': 'TypeScript · Component lifecycle'");
        expect(main).toContain("'frameworks.angular.description': 'Integra AMB Grid nel ciclo di vita del componente Angular, con inizializzazione e cleanup corretti.'");
        expect(main).toContain("'frameworks.angular.description': 'Integrate AMB Grid into the Angular component lifecycle with proper initialization and cleanup.'");
        ...
        expect(main).toContain('<article class="demo-framework-card demo-framework-card--vue card');
        expect(main).toContain('<article class="demo-framework-card demo-framework-card--angular card');
        ...
        expect(frameworkCards).toContain('demo-framework-card__direction-step--3');
        expect(frameworkCards).not.toContain("demoIcon('available'");
        expect(frameworkCards).not.toContain("demoIcon('planned'");
        expect(frameworkCards).not.toContain("demoIcon('arrowUpRight'");
        expect(icons).toContain('ChevronRight');
        expect(icons).toContain('chevronRight: ChevronRight');
        expect(icons).toContain('guide: BookOpen');
        expect(icons).not.toContain('arrowUpRight: ArrowUpRight');
        expect(icons).not.toContain('available: CircleCheck');
        expect(icons).not.toContain('planned: Clock3');
