...
const createDocumentHarness = () => {
    ...
    return {
        ...
        xhrMock: globalThis.XMLHttpRequest,
        restore() {
...
const expectNoAjaxReadSideEffects = (table, crud) => {
...
describe('AMB table controller AJAX URL read API', () => {
    test('exposes getAjaxUrl as a flat read-only controller method', () => {
        ...
        try {
            ...
            const selectedData = [{ id: 1, name: 'Ada' }];
            ...
            expect(typeof controller.getAjaxUrl).toBe('function');
            expect(controller.ajax).toBeUndefined();
            expect(controller.ajaxMethods).toBeUndefined();
            expect(controller.dataMethods).toBeUndefined();
            expect(controller.controllerMethods).toBeUndefined();
            expect(controller.setData).toBeUndefined();
            expect(controller.replaceData).toBeUndefined();
            expect(controller.updateData).toBeUndefined();
            expect(controller.addData).toBeUndefined();
            ...
            const url = controller.getAjaxUrl();

            expect(url).toBe('https://example.test/api/people');
            expect(table.getAjaxUrl).toHaveBeenCalledOnce();
            expect(table.getAjaxUrl).toHaveBeenCalledWith();
            expect(harness.fetchMock).not.toHaveBeenCalled();
            expect(harness.xhrMock).not.toHaveBeenCalled();
            expectNoAjaxReadSideEffects(table, crud);
            ...
            table.getAjaxUrl.mockClear();
            clearTableSideEffects(table);
            clearCrudSetupCalls(crud);

            expect(controller.getAjaxUrl()).toBe('');
            expect(table.getAjaxUrl).toHaveBeenCalledOnce();
            expect(table.getAjaxUrl).toHaveBeenCalledWith();
            expect(harness.fetchMock).not.toHaveBeenCalled();
            expect(harness.xhrMock).not.toHaveBeenCalled();
