...
vi.mock('tabulator-tables', () => ({
    TabulatorFull: class TabulatorMock {
        constructor(selector, options) {
            ...
            this.setColumnLayout = vi.fn(() => true);
            ...
            this.addColumn = vi.fn();
            this.deleteColumn = vi.fn();
            this.updateColumnDefinition = vi.fn();
...
const clearTableSideEffects = table => {
    table.getColumnDefinitions.mockClear();
    table.getColumns.mockClear();
    table.getColumn.mockClear();
    table.setColumns.mockClear();
    table.addColumn.mockClear();
    table.deleteColumn.mockClear();
    table.updateColumnDefinition.mockClear();
    table.moveColumn.mockClear();
    table.showColumn.mockClear();
    table.hideColumn.mockClear();
    table.toggleColumn.mockClear();
...
describe('AMB table controller persistence API', () => {
    test('exposes flat column-layout persistence methods without AMB side effects', () => {
        ...
        try {
            const controller = createTable({
                ...
                columns: [
                    ...
                    { title: 'AMB Actions', field: '_ambActions' }
                ],
                data: [
                    {
                        id: 1,
            ...
            const applyResult = { applied: true };
            ...
            expect(typeof controller.getColumnLayout).toBe('function');
            expect(typeof controller.setColumnLayout).toBe('function');
            expect(controller.persistence).toBeUndefined();
            expect(controller.persistenceMethods).toBeUndefined();
            expect(controller.columnPersistence).toBeUndefined();
            expect(controller.controllerMethods).toBeUndefined();

            expect(controller.setSearchQuery('alice')).toBe(true);
            ...
            table.getColumnLayout.mockReturnValueOnce(layout);
            const returnedLayout = controller.getColumnLayout();

            expect(returnedLayout).toBe(layout);
            expect(returnedLayout[0]).toBe(groupLayout);
            expect(returnedLayout[0].columns).toBe(groupLayout.columns);
            expect(returnedLayout[0].columns[0]).toBe(childLayout);
            expect(returnedLayout[1]).toBe(actionLayout);
            expect(returnedLayout[1].field).toBe('_ambActions');
            expect(table.getColumnLayout).toHaveBeenCalledOnce();
            expect(table.getColumnLayout).toHaveBeenCalledWith();

            table.setColumnLayout.mockReturnValueOnce(applyResult);
            expect(controller.setColumnLayout(layout)).toBe(applyResult);
            expect(table.setColumnLayout).toHaveBeenCalledOnce();
            expect(table.setColumnLayout).toHaveBeenCalledWith(layout);
            ...
            expect(table.setColumnLayout.mock.calls[0][0][1]).toBe(actionLayout);
            ...
            expect(table.updateColumnDefinition).not.toHaveBeenCalled();
