...
describe('AMB table contextual cell methods', () => {
    test('exposes flat cell methods and delegates through rowMethods.getRowCell', () => {
        ...
        const mutableCell = {
            getField: vi.fn(() => 'name'),
            getOldValue: vi.fn(() => 'Previous'),
            getInitialValue: vi.fn(() => 'Initial'),
            setValue: vi.fn(),
            restoreOldValue: vi.fn(),
            restoreInitialValue: vi.fn()
        };
        const cell = {
            getValue: vi.fn(() => 0),
        ...
        expect(
            methods.restoreCellOldValue(
                rowIdentifier,
                column
            )
        ).toBe(updatedRow);
        expect(
            methods.restoreCellInitialValue(
                rowIdentifier,
                column
            )
        ).toBe(updatedRow);
        ...
        expect(
            mutableCell.restoreOldValue
        ).not.toHaveBeenCalled();
        expect(
            mutableCell.restoreInitialValue
        ).not.toHaveBeenCalled();
        expect(
        ...
        expect(crud.updateRowFields).toHaveBeenCalledTimes(3);
        rowMethods.getRowCell.mockReturnValueOnce({
            getField: vi.fn(() => 'name')
        });
        expect(
            methods.restoreCellOldValue(
